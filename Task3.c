...
void SolveTask3() {
    ...
    for ( i = 0; i < nr_cuv-1; i++ ) {
        memcpy(secventa[i], cuv[i], strlen(cuv[i]));
        memcpy((char*)(secventa[i] + strlen(cuv[i])), spatiu, 1);
        memcpy((char*)(secventa[i] + strlen(cuv[i]) + 1), cuv[i+1], strlen(cuv[i+1]));
    ...
    for ( i = 0; i < nr_cuv-1; i++ ) {
        for ( j = 0; j < i; j++ ) {
            if ( strcmp(secventa[j], secventa[i]) == 0 && strcmp(duplicat, secventa[i]) != 0 ) {
